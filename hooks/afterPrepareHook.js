...
function run(cordovaContext) {
  ...
  platformsList.forEach(function(platform) {
    ...
    if (
      pluginPreferences[platform].hosts == null ||
      pluginPreferences[platform].hosts.length == 0
    ) {
      console.warn(
        "No host is specified in the config.xml for " +
          platform +
    ...
    switch (platform) {
      case ANDROID: {
        activateUniversalLinksInAndroid(
          cordovaContext,
          pluginPreferences.android
        );
        break;
      }
      case IOS: {
        activateUniversalLinksInIos(cordovaContext, pluginPreferences.ios);
        break;
      }
    }
  });
